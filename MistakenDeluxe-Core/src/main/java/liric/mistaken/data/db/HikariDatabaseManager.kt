...
class HikariDatabaseManager(
    ...
    private fun connect() {
        ...
        try {
            ...
            MistakenLib.log(MistakenLib.LogCategory.CORE, "[Database] Connected to $type successfully.")
        } catch (e: Exception) {
            MistakenLib.logError(MistakenLib.LogCategory.CORE, "[Database] Failed to connect to $type: ${e.message}")
            throw e
        }
    }
    ...
    override fun close() {
        if (::dataSource.isInitialized && !dataSource.isClosed) {
            dataSource.close()
            MistakenLib.log(MistakenLib.LogCategory.CORE, "[Database] Connection pool closed.")
        }
    }
}
