 ...
 */
class MessageConfig(private val plugin: Mistaken) : liric.mistaken.api.managers.IMessageConfig {
            ...
            runCatching {
                ...
                JarFile(jarFile).use { jar ->
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        // langs/ → archivos de traducción por idioma
                        // menus/ → archivos globales de layout (uno por menú, sin duplicar por lang)
                        val isLangFile = path.startsWith("langs/") && path.endsWith(".yml") && !entry.isDirectory
                        val isMenuFile = path.startsWith("menus/") && path.endsWith(".yml") && !entry.isDirectory

                        if (isLangFile || isMenuFile) {
    ...
    override fun getMessage(player: Player?, path: String, vararg extraTags: TagResolver): Component {
    ...
    /**
     * Devuelve una lista de strings SIN parsear (raw) desde messages.yml del jugador.
     * Útil para MenuBase.getTranslatedList() donde la subclase quiere llamar mm.deserialize() ella misma.
     * Hace fallback al idioma por defecto si la lista está vacía.
     */
