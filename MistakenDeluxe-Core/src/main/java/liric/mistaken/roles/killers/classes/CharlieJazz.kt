...
class CharlieJazz : CoreKiller(
    "charliejazz",
    MessageService.getStrictString(null, "killers.charliejazz.name", "killers_info")
) {
    ...
    private fun preLoadKit() {
        ...
        val armorKeys = listOf("helmet", "chestplate", "leggings", "boots")
        val itemKeys = listOf("weapon", "skill1", "skill2", "skill3", "skill4")
    ...
    override fun showPhysicalTrail(player: Player) {
        ...
        val entidades = orbitadores.getOrPut(uuid) {
            orbitMaterials.map { mat ->
                PacketFactory.displays.buildBlockDisplay(player.sessionViewers(), player.location) { bd ->
    ...
    override fun showTrail(player: Player) {
        ...
        loc.world.players.forEach {
            if (it.location.distanceSquared(loc) < 2500) {
                it.spawnParticle(org.bukkit.Particle.END_ROD, loc.x, loc.y + 1.0, loc.z, 2, 0.15, 0.15, 0.15, 0.02)
            }
        }
    }
