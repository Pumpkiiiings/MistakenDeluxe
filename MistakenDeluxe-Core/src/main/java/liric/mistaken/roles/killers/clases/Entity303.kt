...
class Entity303 : CoreKiller(
    ...
    override fun useSkill(player: Player, slot: Int) {
        when (slot) {
            1 -> if (!checkCooldown(player, 1)) { habilidadDashCodigo(player); playSkillEffects(player, 1) }
            2 -> if (!checkCooldown(player, 2)) { habilidadInfeccionSistema(player); playSkillEffects(player, 2) }
            3 -> if (!checkCooldown(player, 3)) { habilidadProtocoloVuelo(player); playSkillEffects(player, 3) }
            4 -> if (!checkCooldown(player, 4)) { habilidadCrashPantalla(player); playSkillEffects(player, 4) }
        }
    }
    ...
    override fun equip(player: Player) {
        ...
        fun deliver(key: String, slot: Int, isArmor: Boolean = false) {
            ...
            val namePath = if (key == "weapon") "asesinos.entity303.skill_names.weapon"
