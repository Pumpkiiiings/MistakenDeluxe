...
import liric.mistaken.roles.asesinos.clases.*
 ...
 */
class AsesinoManager(private val plugin: Mistaken) : liric.mistaken.api.managers.IAsesinoManager {
    ...
    override val catalogo: Map<String, Asesino> get() = clasesDisponibles
    ...
    override fun registrarClase(asesino: Asesino) {
    ...
    fun registrarAsesino(player: Player, asesino: Asesino) {
        ...
        player.scheduler.runDelayed(
            ...
            Consumer { _ ->
                ...
                val weaponSlot = config.getInt("$pathBase.items.arma_slot", 8)
                if (weaponSlot != 8) {
    ...
    // --- GETTERS ---
    override fun getClasePorId(id: String?): Asesino? = id?.lowercase()?.let { clasesDisponibles[it] }
