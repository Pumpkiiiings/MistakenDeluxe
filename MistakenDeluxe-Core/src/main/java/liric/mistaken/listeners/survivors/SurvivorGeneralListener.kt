...
/**
 * [LIRIC-MISTAKEN 2.0]
 * SurvivorGeneralListener: Protección de inventario y equipo.
 * FIX: Se agregaron bloqueos contra teclas numéricas rápidas.
 */
class SurvivorGeneralListener(private val plugin: Mistaken) : Listener {
    ...
    fun onInventoryClick(event: InventoryClickEvent) {
        ...
        // 1. Filtro O(1) rápido
        if (!plugin.supervivienteManager.esSurvivorActivo(player)) return
    ...
    fun onPlayerSwapHand(event: PlayerSwapHandItemsEvent) {
        if (plugin.supervivienteManager.esSurvivorActivo(event.player)) {
            event.isCancelled = true
        }
    }
}
