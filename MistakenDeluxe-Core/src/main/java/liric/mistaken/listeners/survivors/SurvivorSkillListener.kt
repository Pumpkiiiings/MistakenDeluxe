...
class SurvivorHabilidadListener(private val plugin: Mistaken) : Listener {
    ...
    fun onSurvivorMeleeAttack(event: EntityDamageByEntityEvent) {
        ...
        if (pdc.has(STICK_KEY, PersistentDataType.BYTE) && clase is RaincoatKid) {
            val cooldownTime = plugin.configManager.getSurvivorConfig(this.id).getInt("supervivientes.raincoatkid.items.skill3_cooldown", 40)
            if (!clase.checkCooldown(attacker, 2, cooldownTime)) {
                clase.aplicarGolpePalo(victim)
                attacker.sendMessage(plugin.mm.deserialize("<green><bold>�BAM!</bold> <gray>Killer aturdido."))
        ...
        if (pdc.has(JESSE_PUNCH_KEY, PersistentDataType.BYTE) && clase is Jesse) {
            val cooldownTime = plugin.configManager.getSurvivorConfig(this.id).getInt("supervivientes.jesse.items.skill2_cooldown", 15)
    ...
    fun onProjectileHit(event: ProjectileHitEvent) {
        ...
        if (pdc.has(EMERALD_KEY, PersistentDataType.BYTE)) {
            ...
            if (isKiller) {
                ...
                victim.sendMessage(plugin.mm.deserialize("<green>�Ooh! �Una esmeralda! (Te has distra�do)"))
            }
            return
        }
    }
