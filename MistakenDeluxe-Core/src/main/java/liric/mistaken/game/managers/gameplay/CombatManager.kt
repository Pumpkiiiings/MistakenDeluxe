            ...
            target.scheduler.run(plugin, Consumer { _ ->
                plugin.server.onlinePlayers.forEach { viewer ->
                    if (target.isOnline && viewer.isOnline) {
                        try {
                            plugin.glowingAPI.unsetGlowing(target, viewer)
                        } catch (_: Exception) {
                        }
                        try {
                            plugin.glowingAPI.unsetGlowing(viewer, target)
                        } catch (_: Exception) {
                        }
                    }
                }
            ...
            for (p1 in plugin.server.onlinePlayers) {
                p1.scheduler.run(plugin, Consumer { _ ->
                    for (p2 in plugin.server.onlinePlayers) {
                        try {
                            plugin.glowingAPI.unsetGlowing(p1, p2)
                        } catch (_: Exception) {
                        }
                        try {
                            plugin.glowingAPI.unsetGlowing(p2, p1)
                        } catch (_: Exception) {
                        }
                    }
                }, null)
