...
class RomeoProfile : CinematicProfile {
    ...
    override fun playEffects(plugin: Mistaken, loc: Location, dummy: ArmorStand, isIntro: Boolean, displayManager: DisplayManager, viewers: List<Player>) {
        ...
        viewers.forEach { p ->
            if (isIntro) {
                liric.mistaken.utils.hooks.ObserverHook.playScreenTint(p, 255, 0, 0, 0.7f, 60)
                liric.mistaken.utils.hooks.ObserverHook.playScreenshake(p, 0.8f, 40)
                p.playSound(loc, org.bukkit.Sound.BLOCK_END_PORTAL_SPAWN, 1f, 0.5f)
            } else {
                liric.mistaken.utils.hooks.ObserverHook.playScreenTint(p, 255, 0, 0, 0.8f, 80)
