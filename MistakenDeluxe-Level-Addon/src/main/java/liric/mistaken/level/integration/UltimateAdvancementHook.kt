...
class UltimateAdvancementHook(private val plugin: LevelAddonPlugin) {
    ...
    init {
        if (Bukkit.getPluginManager().isPluginEnabled("UltimateAdvancementAPI")) {
            isEnabled = true
            plugin.componentLogger.info(liric.mistaken.utils.color.ColorTranslator.translate("<green>[SUCCESS]</green> <gray>UltimateAdvancementAPI hooked via reflection.</gray>"))
            try {
                setupReflection()
