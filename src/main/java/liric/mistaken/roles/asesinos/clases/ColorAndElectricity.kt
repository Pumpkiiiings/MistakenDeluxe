...
class ColorAndElectricity : Asesino(
    ...
    private fun preLoadKit() {
        ...
        armor.forEach { k ->
            config.getString("$path.armadura.$k")?.let { id ->
                if (id != "none") {
                    val item = CraftEngineHook.getCustomItem(id) ?: ItemStack(Material.matchMaterial(id) ?: Material.LEATHER_HELMET)
                    itemKitCache[k] = item
                }
            }
        }
    ...
    private fun habilidadShikisaiEnd(player: Player) {
        player.scheduler.run(plugin, { _ ->
            ...
            val target = player.world.getNearbyPlayers(player.location, 15.0).find { esObjetivoValido(player, it) }
            target?.let { t ->
    ...
    override fun mostrarTrailFisico(player: Player) {
        ...
        if (orbitadores[uuid]?.firstOrNull()?.world != playerLoc.world) limpiarEntidades(uuid)
