 ...
 * FIX: Delegación de Region y Entity Schedulers (Folia Ready).
 */
class Entity303 : Asesino(
    ...
    private fun preLoadKit() {
        ...
        armor.forEach { k ->
            config.getString("$path.armadura.$k")?.let { id ->
                if (id != "none") {
                    val item = CraftEngineHook.getCustomItem(id) ?: ItemStack(Material.matchMaterial(id) ?: Material.NETHERITE_HELMET)
                    itemKitCache[k] = item
                }
            }
        }
    ...
    private fun habilidadInfeccionSistema(player: Player) {
        ...
        val dir = startLoc.direction.multiply(1.5)
        ...
        plugin.server.regionScheduler.run(plugin, startLoc, { _ ->
            ...
            val task = plugin.server.globalRegionScheduler.runAtFixedRate(plugin, Consumer { task ->
                ...
                val currentLoc = star.location.add(dir)
                star.teleport(currentLoc)
                currentLoc.world.spawnParticle(org.bukkit.Particle.ENCHANT, currentLoc, 5, 0.1, 0.1, 0.1, 0.05)

                val hit = currentLoc.world.getNearbyPlayers(currentLoc, 1.2).firstOrNull { esObjetivoValido(player, it) }

                if (hit != null || currentLoc.block.type.isSolid) {
                    currentLoc.world.spawnParticle(org.bukkit.Particle.EXPLOSION, currentLoc, 1)
                    hit?.scheduler?.run(plugin, { _ ->
                        ...
                        hit.addPotionEffect(PotionEffect(PotionEffectType.HUNGER, 100, 1))
                    }, null)
                    star.remove()
                    task.cancel()
                }
    ...
    private fun habilidadCrashPantalla(player: Player) {
        ...
        plugin.server.onlinePlayers.filter { it.world == playerLoc.world && it.location.distanceSquared(playerLoc) < 1600 }.forEach { online ->
    ...
    override fun mostrarTrailFisico(player: Player) {
        ...
        if (plugin.asesinoManager?.esElAsesino(player) != true) { limpiar(uuid); return }

        val playerLoc = player.location
        if (orbitadores[uuid]?.firstOrNull()?.world != playerLoc.world) limpiar(uuid)
        ...
        val entidades = orbitadores.getOrPut(uuid) {
            orbitMaterials.mapNotNull { mat ->
                ...
                plugin.server.regionScheduler.run(plugin, playerLoc, { _ ->
                    display = crearBloqueOrbitante(playerLoc, mat)
                })
                display
            }.toMutableList()
        }
