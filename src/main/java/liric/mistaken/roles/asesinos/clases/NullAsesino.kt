 ...
 * MEJORAS: Schedulers Folia, Finishers Aislados.
 */
class NullAsesino : Asesino(
    ...
    private fun habilidadPrisionVacio(player: Player) {
        player.scheduler.run(plugin, { _ ->
            ...
            val victim = ray?.hitEntity as? Player ?: return@run
            ...
            mutableListOf<ItemDisplay>().apply {
                orbitMaterials.forEach { mat ->
                    plugin.server.regionScheduler.run(plugin, playerLoc, { _ ->
                        add(crearItemOrbitante(playerLoc, mat))
                    })
                }
            }
        }
    ...
    private fun crearItemOrbitante(loc: Location, mat: Material): ItemDisplay {
        return loc.world.spawn(loc, ItemDisplay::class.java) { id ->
            ...
            id.teleportDuration = 3; id.interpolationDuration = 3
        }
    }
    ...
    private fun cleanupTrap(trap: Entity) {
        plugin.server.regionScheduler.run(plugin, trap.location, { _ -> trap.remove() })
    ...
    override fun cleanup(player: Player?) {
        ...
        player?.let {
            ...
            it.scheduler.run(plugin, { _ -> it.getAttribute(Attribute.SCALE)?.baseValue = 1.0 }, null)
            lastKillEffect.remove(it.uniqueId)
        }
        activeTraps.forEach { cleanupTrap(it) }
        activeTraps.clear()
    }
}
