...
/**
 *[LIRIC-MISTAKEN 2.0]
 * SupervivienteHabilidadListener: Manejo de items adaptado a MULTIARENA.
 */
class SupervivienteHabilidadListener(private val plugin: Mistaken) : Listener {
    ...
    fun onUseSurvivorAbility(event: PlayerInteractEvent) {
        ...
        if (plugin.combatManager?.isFrozen(player) == true) return
    ...
    fun onSurvivorMeleeAttack(event: EntityDamageByEntityEvent) {
        ...
        if (session.esAsesino(attacker.uniqueId) || !session.esAsesino(victim.uniqueId)) return
        if (attacker.gameMode != GameMode.SURVIVAL || attacker.isInvisible) return
        if (plugin.combatManager?.isFrozen(attacker) == true) return
