...
object MistakenTestCommand {
    ...
    fun get(plugin: Mistaken): LiteralCommandNode<CommandSourceStack> {
        ...
        // --- IGNORAR JUGADORES ---
        rootNode.then(Commands.literal("ignore")
        ...
        // --- GEOFFREY ---
        rootNode.then(Commands.literal("geoffrey")
        ...
        // --- AMONG US (SUS) ---
        rootNode.then(Commands.literal("amongus")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        // --- POU ---
        rootNode.then(Commands.literal("pou")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        // --- AXOLOTL ---
        rootNode.then(Commands.literal("axolotl")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        // --- OBSERVANT ---
        rootNode.then(Commands.literal("observant")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        // --- EYEDROOMS ---
        rootNode.then(Commands.literal("eyedrooms")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        // --- WITHER STORM ---
        rootNode.then(Commands.literal("witherstorm")
            .then(Commands.literal("start").executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
        ...
        rootNode.then(Commands.literal("spawnall")
            .executes { ctx ->
                val p = ctx.source.sender as? Player ?: return@executes 0
                val loc = p.location
        ...
        // --- STOP ---
        rootNode.then(Commands.literal("stop")
            .executes { ctx ->
                val sender = ctx.source.sender
