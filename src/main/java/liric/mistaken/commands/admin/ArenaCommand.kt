 ...
 * FIX: Safe-Calls para evitar crasheos si se ejecuta en un Lobby.
 */
class ArenaCommand(private val plugin: Mistaken) : BasicCommand {
    ...
    override fun execute(stack: CommandSourceStack, args: Array<String>) {
        ...
        if (arenaManager == null) {
            sender.sendMessage(plugin.mm.deserialize("<red>El sistema de arenas está desactivado en este servidor (Modo Lobby)."))
            return
        }
        ...
        when (sub) {
            ...
            "setspawn" -> {
                ...
                if (type == "asesino" || type == "survivor") {
                    arenaManager.setSpawn(arenaName, type, player.location)
            ...
            "setgenerator" -> {
                ...
                arenaManager.addGenerator(arenaName, target.location)
            ...
            "delgenerator" -> {
                ...
                arena?.let {
                    ...
                    if (removed) {
                        arenaManager.saveGenerators(arenaName, currentGens)
    ...
    override fun suggest(stack: CommandSourceStack, args: Array<String>): List<String> {
        ...
        val arenaManager = plugin.arenaManager ?: return emptyList()

        return when (args.size) {
            1 -> listOf("create", "delete", "check", "setspawn", "setgenerator", "delgenerator")
                .filter { it.startsWith(args[0], true) }
            2 -> arenaManager.getArenas().keys
