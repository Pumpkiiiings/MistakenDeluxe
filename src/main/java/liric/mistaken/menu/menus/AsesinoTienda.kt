...
class AsesinoTienda : MenuBase("asesinos_tienda") {
    ...
    override fun setupItems(player: Player, gui: Gui, config: FileConfiguration) {
        val asesinoManager = plugin.asesinoManager ?: return // Si estamos en Lobby de espera, la tienda puede no cargar info del juego
        ...
        val asesinosCatalogo = asesinoManager.getClasesDisponibles().keys
        ...
        for (killerId in asesinosCatalogo) {
            ...
            // Aplicar Flags de forma segura en Paper
    ...
    private fun handlePurchaseLogic(player: Player, killerId: String, precio: Int, tiene: Boolean) {
        ...
        if (tiene) {
            ...
            // NamespaceKey es una propiedad global lazy de la clase principal
